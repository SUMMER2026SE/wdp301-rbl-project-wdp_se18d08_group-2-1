...
import { Bell, Bookmark, Camera, CheckCircle2, ChevronRight, Clock, Heart, Image, MapPin, MessageCircle, MoreHorizontal, PenLine, Pin, Search, Send, Share2, ShieldCheck, Sparkles, Star, Tag, TrendingUp, Users, X } from "lucide-react";
...
const c = {
  vi: {
    member: "Th\u00e0nh vi\u00ean", composer: "B\u1ea1n mu\u1ed1n chia s\u1ebb \u0111i\u1ec1u g\u00ec v\u1edbi c\u1ed9ng \u0111\u1ed3ng PhotoHub?", photo: "\u1ea2nh m\u1eabu", ask: "H\u1ecfi \u00fd ki\u1ebfn", story: "K\u1ec3 job",
  ...
  en: {
    member: "Member", composer: "What would you like to share with the PhotoHub community?", photo: "Sample photo", ask: "Ask opinion", story: "Job story",
...
function Avatar({ user, text, size = "h-11 w-11" }) {
  const initial = user?.fullName?.charAt(0)?.toUpperCase() || user?.name?.charAt(0)?.toUpperCase() || "P";
  const getUrl = () => {
    if (!user?.avatar) return null;
    if (user.avatar.startsWith("http://") || user.avatar.startsWith("https://")) return user.avatar;
    return `http://localhost:3000${user.avatar.startsWith("/") ? user.avatar : `/${user.avatar}`}`;
  };
  const avatarUrl = getUrl();
  return avatarUrl
    ? <img src={avatarUrl} alt={user.fullName || user.name || text.member} className={`${size} rounded-full object-cover ring-2 ring-white`} />
    : <div className={`${size} flex items-center justify-center rounded-full bg-orange-100 text-sm font-black text-orange-700 ring-2 ring-white`}>{initial}</div>;
}
...
function Composer({ user, onSubmit, categories, text }) {
  ...
  return <section className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
    ...
    <div className="mt-4 grid grid-cols-3 gap-2 border-t border-slate-100 pt-3 text-xs font-bold text-slate-600">
