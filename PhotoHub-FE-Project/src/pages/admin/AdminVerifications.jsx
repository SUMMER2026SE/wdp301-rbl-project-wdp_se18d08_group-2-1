...
export default function AdminVerifications() {
  ...
  const [status, setStatus] = useState("PENDING"); // Mặc định lọc các hồ sơ PENDING
    ...
    <div className="space-y-6 animate-fadeIn">
      ...
      <div className="flex border-b border-slate-800 gap-4">
        {[
          ...
          { label: "Đã phê duyệt", val: "APPROVED" },
          { label: "Đã từ chối", val: "REJECTED" }
        ].map(tab => (
          <button
            key={tab.val}
            onClick={() => { setStatus(tab.val); setPage(1); setSelectedVerification(null); }}
                ...
                verifications.map((v) => (
                  <tr key={v._id} className="hover:bg-slate-800/10 transition">
                    <td className="py-3 px-3">
                      <div className="font-semibold text-white">{v.photographer?.displayName || "Nhiếp ảnh gia"}</div>
                      <div className="text-xs text-slate-500">{v.user?.email}</div>
                    </td>
                    <td className="py-3 px-3 text-slate-300 font-mono">{v.idCardNumber}</td>
          ...
          {selectedVerification ? (
            <div className="space-y-4 text-sm">
              <div>
                ...
                <p className="text-white font-semibold text-base mt-0.5">{selectedVerification.user?.fullName}</p>
                <p className="text-slate-500 text-xs">{selectedVerification.user?.email}</p>
              </div>

              <div>
                <span className="text-slate-400 text-xs">Số CMND/CCCD:</span>
                <p className="text-white font-semibold font-mono">{selectedVerification.idCardNumber}</p>
              </div>
              ...
              <div className="space-y-2 pt-2">
                ...
                <div className="grid grid-cols-2 gap-2">
                  <div className="border border-slate-800 rounded-lg overflow-hidden bg-slate-950">
                    ...
                    <img 
                      src={selectedVerification.idCardFrontImage} 
                      alt="Front card" 
                      className="w-full h-24 object-cover hover:scale-105 transition duration-300 cursor-pointer"
                      onClick={() => Swal.fire({ imageUrl: selectedVerification.idCardFrontImage, imageAlt: "Mặt trước CCCD", background: "#0f172a" })}
                    />
                  </div>
                  <div className="border border-slate-800 rounded-lg overflow-hidden bg-slate-950">
                    <p className="text-[10px] text-slate-500 text-center py-1">Mặt sau</p>
                    <img 
                      src={selectedVerification.idCardBackImage} 
                      alt="Back card" 
                      className="w-full h-24 object-cover hover:scale-105 transition duration-300 cursor-pointer"
                      onClick={() => Swal.fire({ imageUrl: selectedVerification.idCardBackImage, imageAlt: "Mặt sau CCCD", background: "#0f172a" })}
                    />
                  </div>
                </div>
              </div>
                ...
                <div className="flex justify-between">
                  ...
                  <span className={`font-semibold ${
                    selectedVerification.status === "APPROVED" 
                      ...
                      ? "text-red-400" 
                      : "text-yellow-500"
                  }`}>{selectedVerification.status}</span>
                </div>
                {selectedVerification.status !== "PENDING" && (
                  <>
                    <div className="flex justify-between">
                      <span className="text-slate-400">Người duyệt:</span>
                      <span className="text-white font-medium">{selectedVerification.reviewedBy?.fullName || "Admin"}</span>
                    </div>
                    <div>
                      ...
                      <p className="text-white bg-slate-950 p-2.5 rounded-lg border border-slate-800 text-xs mt-1 italic">
                        {selectedVerification.adminNote}
                      </p>
                    </div>
                  </>
                )}
